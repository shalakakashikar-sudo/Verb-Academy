...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-24 px-4 sm:px-6 pt-12 flex flex-col items-center">
      <header className="w-full max-w-5xl flex flex-col items-center mb-16">
        <h1 className="text-5xl md:text-7xl font-cinzel text-amber-400 drop-shadow-[0_5px_15px_rgba(251,191,36,0.4)] mb-4 text-center tracking-tighter uppercase">
          Verb Academy
        </h1>
        <p className="text-slate-300 text-center max-w-2xl text-lg md:text-xl font-medium leading-relaxed px-4">
          Master the ancient treasures of language with your guardian Hedgie.
        </p>
      </header>

      <main className="w-full max-w-5xl glass-panel rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 shadow-2xl relative transition-all duration-500">
        ...
        <div className="xl:hidden mb-12 flex justify-center">
          <HedgehogMascot mood={mascotMood} externalMessage={mascotMessage} />
        </div>

        {view === 'selection' && (
          <div className="space-y-8 animate-fadeIn">
            <h2 className="text-3xl font-cinzel text-amber-300 border-b-2 border-amber-500/20 pb-4 mb-6">Imperial Scrolls</h2>
            <div className="grid gap-6 md:grid-cols-2">
              {LESSONS.map((lesson) => (
                <button
                  key={lesson.id}
                  onClick={() => handleSelectLesson(lesson)}
                  className="group relative flex items-center p-6 md:p-8 bg-slate-900/40 hover:bg-amber-500/5 border border-white/10 hover:border-amber-400 rounded-3xl transition-all duration-300 text-left shadow-lg hover:shadow-amber-400/10 h-full"
                >
                  <div className="flex-1">
                    <h3 className="text-2xl font-bold text-amber-400 group-hover:text-amber-300 transition-colors">{lesson.title}</h3>
                    <p className="text-slate-400 text-base mt-2 font-medium leading-relaxed">{lesson.description}</p>
                  </div>
                  <div className="ml-4 transform group-hover:scale-125 transition-transform duration-300">
        ...
        {view === 'lesson' && currentLesson && (
          <div className="animate-fadeIn">
            <button 
              ...
              className="group text-amber-400 hover:text-amber-300 flex items-center gap-2 mb-8 text-sm font-bold uppercase tracking-widest transition-all"
            >
        ...
        {view === 'quiz' && currentLesson && (
          <div className="animate-fadeIn">
            ...
            <h2 className="text-4xl font-cinzel text-amber-400 mb-10 text-center tracking-tight">The Trial of Verbs</h2>
            <QuizView 
